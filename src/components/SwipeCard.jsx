...
const SwipeCard = ({ place, index, onSwipe, onSwipeUp, topCardX, onTopCardXChange, direction = 0 }) => {
  ...
  const handleDragEnd = async (event, info) => {
    ...
    if (Math.abs(offset.y) > 120 && offset.y < 0 && Math.abs(offset.x) < 50) {
      setIsExiting(true);
    ...
    if (Math.abs(offset.x) > threshold) {
      ...
      // Exit animation with velocity carry-over
      await controls.start({
        x: finalX,
        y: yOffset,
  ...
  return (
    <motion.div
      ...
      style={{ 
        ...
        // Top card always scale 1, cards underneath use reactive scale
